...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Socrates workspace utilities')
    subparsers = parser.add_subparsers(dest='command', required=True)
    subparsers.add_parser('summary', help='render a Markdown summary of the Python workspace')
    ...
    commands_parser = subparsers.add_parser('commands', help='list command inventory entries')
    ...
    tools_parser = subparsers.add_parser('tools', help='list tool inventory entries')
    ...
    route_parser = subparsers.add_parser('route', help='route a prompt across command/tool inventories')
    route_parser.add_argument('prompt')
    route_parser.add_argument('--limit', type=int, default=5)

    bootstrap_parser = subparsers.add_parser('bootstrap', help='build a runtime-style session report from the command/tool inventories')
    bootstrap_parser.add_argument('prompt')
    bootstrap_parser.add_argument('--limit', type=int, default=5)

    loop_parser = subparsers.add_parser('turn-loop', help='run a small stateful turn loop for the runtime')
    ...
    show_command = subparsers.add_parser('show-command', help='show one command entry by exact name')
    show_command.add_argument('name')
    show_tool = subparsers.add_parser('show-tool', help='show one tool entry by exact name')
    show_tool.add_argument('name')

    exec_command_parser = subparsers.add_parser('exec-command', help='execute a command shim by exact name')
    exec_command_parser.add_argument('name')
    exec_command_parser.add_argument('prompt')

    exec_tool_parser = subparsers.add_parser('exec-tool', help='execute a tool shim by exact name')
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == 'commands':
        if args.query:
            ...
            output_lines.extend(f'- {module.name} 鈥?{module.source_hint}' for module in commands[: args.limit])
            print('\n'.join(output_lines))
        return 0
    if args.command == 'tools':
        if args.query:
            ...
            output_lines.extend(f'- {module.name} 鈥?{module.source_hint}' for module in tools[: args.limit])
            print('\n'.join(output_lines))
        return 0
