...
def resolve_managed_settings_path() -> Path | None:
    ...
    if system == "Darwin":
        managed = Path("/Library/Application Support/Socrates/managed-settings.json")
        if managed.exists():
            return managed

    if system == "Linux":
        managed = Path("/etc/socrates/managed-settings.json")
        if managed.exists():
            return managed

    if system == "Windows":
        program_data = os.environ.get("ProgramData", r"C:\ProgramData")
        managed = Path(program_data) / "Socrates" / "managed-settings.json"
        if managed.exists():
            return managed
