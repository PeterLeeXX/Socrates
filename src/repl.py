...
class SocratesCLI:
    ...
    def print_banner(self):
        ...
        if HAS_RICH:
            ...
            banner = f"""
{self._colorize('+--------------------------------------------------------+', 'cyan')}
{self._colorize('|', 'cyan')}   {self._colorize('Socrates', 'bold')} - Claude Code Reimplementation   {self._colorize('|', 'cyan')}
...
"""
    ...
    def handle_command(self, user_input: str) -> bool:
        ...
        if cmd in ('exit', 'quit', 'q'):
            print(f"\n{self._colorize('Goodbye!', 'green')} 棣冩啟\n")
            return False
    ...
    def run(self):
        ...
        print(f"{self._colorize('閳光偓' * 60, 'dim')}\n")

        while self.running:
            try:
                user_input = input(f"{self._colorize('socrates>', 'cyan')} ").strip()
                if user_input:
