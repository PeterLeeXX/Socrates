...
try:
    ...
    if not hasattr(_pt_ansi_seq, "_socrates_shift_enter_registered"):
        ...
        _pt_ansi_seq._socrates_shift_enter_registered = True  # type: ignore[attr-defined]
    try:
...
# ``reference UI components/TaskListV2.tsx`` for the reference UI.
...
@dataclass
class PermissionChoiceState:
    """State for the compact permission confirmation control."""

    selected_index: int = 0
    labels: tuple[str, str] = ("Allow once", "Deny")
...
class SocratesREPL:
    ...
    def __init__(
        ...
        *,
        ...
        if not config.get("api_key"):
            ...
            self.console.print("Run [bold]socrates login[/bold] to configure.")
            sys.exit(1)
        ...
        history_file = Path.home() / ".socrates" / "history"
        ...
        self.prompt_session = PromptSession(
            ...
            style=Style.from_dict({
                # Dim background on the prompt marker so the user
    ...
    def _handle_permission_request(
        ...
        with self._permission_prompt_lock:
            ...
            with self._permission_terminal_scope():
                self._render_permission_prompt(message)
                allowed = self._select_permission_choice()
            if allowed and can_enable_setting:
                self._enable_permission_setting(setting_to_enable)
            self._permission_decision_cache[cache_key] = allowed
            return allowed, False

    @contextmanager
    def _permission_terminal_scope(self):
        """Give the permission UI exclusive ownership of the terminal.

        Permission checks are often raised while ``LiveStatus`` is mounted at
        the bottom of the terminal. The confirmation box is another
        prompt_toolkit application, so the live region must be paused before
        we render either the Rich explanation panel or the key-driven choice
        control.
        """
    ...
    def _render_permission_prompt(self, message: str) -> None:
        """Render the permission request as one stable Rich block."""
        ...
        body = Text()
        body.append(str(message).strip() or "A tool is requesting permission.")
        body.append("\n\n")
        body.append("Choose with left/right, then press Enter.", style="dim")
        self.console.print("")
        self.console.print(
            Panel(
                body,
    ...
    def _select_permission_choice(self) -> bool:
        """Return True for Allow once, False for Deny."""
        if not _HAS_PROMPT_TOOLKIT:
            return self._select_permission_choice_text_fallback()
        try:
        ...
        selected: list[bool] = [state.allows]

        def _render() -> FormattedText:
            parts: list[tuple[str, str]] = [
                ("class:title", "+ Permission "),
                ("", " "),
            ]
            for idx, label in enumerate(state.labels):
                style = "class:selected" if idx == state.selected_index else "class:option"
                parts.append((style, f" {label} "))
                if idx < len(state.labels) - 1:
                    parts.append(("", "  "))
            parts.append(("class:hint", "  left/right, enter"))
        ...
        @bindings.add("left")
        @bindings.add("h")
        def _left(event):  # type: ignore[no-untyped-def]
            state.move_left()
        ...
        @bindings.add("l")
        @bindings.add("tab")
        def _right(event):  # type: ignore[no-untyped-def]
            state.move_right()
            event.app.invalidate()
        ...
        def _confirm(event):  # type: ignore[no-untyped-def]
            selected[0] = state.allows
            event.app.exit()

        @bindings.add("escape")
        @bindings.add("c-c")
        def _deny(event):  # type: ignore[no-untyped-def]
            selected[0] = False
            event.app.exit()
    ...
    def _select_permission_choice_text_fallback(self) -> bool:
        """Fallback for tests, pipes, or terminals without prompt_toolkit."""
        choice = self._safe_input("[1] Allow once  [2] Deny > ").strip().lower()
        return choice in ("1", "allow", "allow once", "y", "yes", "")
        ...
        try:
            for cmd in self.command_registry.list_commands():
                ...
                if cmd_name in self._original_built_ins:
                    continue
                add_command(cmd_name, cmd.description, "command")
        except Exception:
            pass
    ...
    def _safe_input(self, prompt: str) -> str:
        ...
        the read. Two prompt_toolkit Applications cannot share a TTY -        without pausing, the spinner row keeps redrawing and shreds the
        user's keystrokes.
        """
    ...
    def _print_startup_header(self):
        ...
        if Panel is None or Group is None or Align is None or Table is None or Text is None or Columns is None:
            print(mascot_ascii)
            print(f"Socrates v{__version__}")
        ...
        table.add_row("Version", Text.assemble(("Socrates", "bold white"), ("  ", ""), (f"v{__version__}", "bold cyan")))
    ...
    def _try_run_skill_slash(self, raw: str) -> bool:
        ...
        try:
            self._permission_decision_cache["skill"] = True
...
"""
    ...
    def _handle_skill_command(self) -> None:
        ...
        try:
            ...
            if not skills:
                ...
                self.console.print("[dim]Create skills in ~/.socrates/skills/ or ~/.claude/skills/ or .socrates/skills/ in your project.[/dim]")
                return
    ...
    def _is_recoverable_tool_error(self, tool_name: str, tool_output) -> bool:
        ...
        if name == "read" and e.startswith("file not found:"):
            ...
            if "/.socrates/skills/" in p or "\\.socrates\\skills\\" in p or "/.claude/skills/" in p or "\\.claude\\skills\\" in p:
                return True
        return False
    ...
    def _should_try_direct_stream(self, user_input: str) -> bool:
        ...
        code_task_markers = (
            ...
            "analyze", "build", "debug", "implement", "review", "config", "script",
            "dependency", "import", "error", "traceback", "pytest", "docs",
        )
    ...
    def chat(self, user_input: str, max_turns: int | None = None):
        ...
        try:
            ...
            if self._should_try_direct_stream(user_input):
                ...
                with (_pt_patch_stdout(raw=True) if sys.stdout.isatty() else nullcontext()):
                    with LiveStatus(
                        ...
                        if isinstance(content, list):
                            for block in content:
                                if isinstance(block, ToolResultBlock):
                                    ...
                                    # ``  - `` (see
                                    # ``reference UI components/MessageResponse.tsx``).
                                    if block.is_error:
                                        err_text = block.content if isinstance(block.content, str) else str(block.content)
                                        self.console.print(f"[red]  - {err_text or 'Error'}[/red]")
                                    else:
            ...
            if "401" in error_str or "authentication" in error_str.lower():
                ...
                if choice == "y":
                    ...
                    self.console.print("\n[dim]You can run [bold]socrates login[/bold] later to update your API key.[/dim]")
            else:
                self.console.print(f"\n[red]Error: {e}[/red]")
                import traceback
                traceback.print_exc()
