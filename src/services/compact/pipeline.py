"""
Compression pipeline — orchestrates the compression layers in order.
...
Layers:
  ...
  3. context_collapse          — Read-time projection
...
class CompressionResult:
    ...
    autocompact_result: Any | None = None  # CompactionResult if autocompact ran
...
class PipelineConfig:
    ...
    # Layer 4: post-compact attachment context
    ...
    async def run(
        ...
        if cfg.mc_enabled:
            try:
                ...
                if saved > 0:
                    ...
                    logger.debug("Layer 2 (microcompact): saved %d tokens", saved)
                    if total_saved >= cfg.early_exit_tokens:
                        return CompressionResult(
                            messages=current_messages,
                            tokens_saved=total_saved,
                ...
                logger.warning("Layer 2 (microcompact) failed", exc_info=True)
        ...
        try:
            ...
            if store is not None and store.enabled and store.commits:
                ...
                logger.debug("Layer 3 (context_collapse): projected %d commits", len(store.commits))
        except Exception:
            logger.warning("Layer 3 (context_collapse) failed", exc_info=True)
        ...
        if cfg.provider is not None and cfg.model:
            try:
                ...
                if result is not None:
                    ...
                    logger.debug("Layer 4 (autocompact): saved %d tokens", result.tokens_saved)
            except Exception:
                logger.warning("Layer 4 (autocompact) failed", exc_info=True)
