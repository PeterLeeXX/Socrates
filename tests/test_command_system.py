...
class TestCommandRegistry(unittest.TestCase):
    ...
    def test_registry_does_not_resolve_unregistered_names(self):
        """Command registry should only resolve canonical command names."""
        cmd = PromptCommand(
            name="test",
            description="Test command",
        )
        ...
        self.assertIsNone(self.registry.get("t"))
        self.assertIsNone(self.registry.get("testing"))
...
class TestBuiltinCommands(unittest.TestCase):
    ...
    def test_builtin_commands_do_not_register_aliases(self):
        """Built-in slash commands should only expose their canonical names."""
        registry = CommandRegistry()
        register_builtin_commands(registry)

        for old_alias in ["?", "reset", "new", "quit", "q"]:
            self.assertFalse(registry.has(old_alias))
    ...
    def test_skills_command_with_project_root(self):
        ...
        project_skills_dir = Path(self.tmpdir.name) / ".socrates" / "skills"
