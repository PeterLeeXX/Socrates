...
``PromptSession`` 鈥?we call the bound callbacks directly with a minimal
...
def _make_repl():
    """Construct a SocratesREPL with provider/session/config patched.

    We don't need real config or a live provider 鈥?only the
...
class TestMultilineEntryBindings(unittest.TestCase):
    ...
    def test_xterm_modify_other_keys_shift_enter_sequence_is_registered(self):
        ...
        Enter 鈥?so Shift+Enter in xterm/VSCode with modifyOtherKeys would
...
class TestMultilineSlashCommandRemoved(unittest.TestCase):
    """``/multiline`` is no longer a command 鈥?its behavior is now the
