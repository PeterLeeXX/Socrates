...
def test_summarize_tool_use_for_file_tools() -> None:
    ...
    assert (
        summarize_tool_use("Read", {"file_path": "src/app.py", "offset": 1, "limit": 100})
        == "src/app.py lines 1-100"
    )
...
def test_summarize_tool_use_for_search_tools() -> None:
    assert summarize_tool_use("Glob", {"pattern": "*.py", "path": "src"}) == "*.py in src"
    assert summarize_tool_use("Grep", {"pattern": "needle", "path": "src"}) == "needle in src"
