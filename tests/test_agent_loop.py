...
class TestAgentLoop(unittest.TestCase):
    ...
    def test_agent_loop_stream_emits_final_text_chunks(self):
        ...
        self.assertEqual("".join(chunks), "Hello from Socrates!")
        self.assertEqual(result.response_text, "Hello from Socrates!")
        self.assertEqual(mock_provider.chat.call_count, 1)
        self.assertEqual(len(conversation.messages), 2)
        self.assertEqual(conversation.messages[-1].role, "assistant")
        last_content = conversation.messages[-1].content
        if isinstance(last_content, list):
            self.assertEqual(last_content[0].text, "Hello from Socrates!")
        else:
            self.assertEqual(last_content, "Hello from Socrates!")
