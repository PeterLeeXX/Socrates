"""Full-stack integration tests exercising REPL 鈫?agent loop 鈫?tool dispatch.
...
# ---------------------------------------------------------------------------
# Fake providers 鈥?each drives a specific tool-call scenario
# ---------------------------------------------------------------------------
...
class _ReadWriteEditProvider:
    """Drives a 3-step flow: Read 鈫?Write new file 鈫?Edit existing file."""
    model = "fake"
...
class TestIntegrationREPLChat:
    """End-to-end via SocratesREPL.chat()."""

    def _make_config(self, home: Path):
        cfg_dir = home / ".socrates"
    ...
    def test_repl_chat_simple_query(self, tmp_path):
        ...
        try:
            ...
            with patch("src.repl.core.get_provider_class", return_value=lambda *a, **kw: _TextOnlyProvider()):
                from src.repl.core import SocratesREPL
                repl = SocratesREPL(provider_name="glm", stream=False)
                repl.chat("Hello")
